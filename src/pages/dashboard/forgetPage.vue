...
export default {
  ...
  methods: {
    ...
    nextStep() {
      if (this.form.mobileNum == "" || this.form.pass == "" || this.form.passAgain == "") {
        this.$Message.warning("每一项都不能为空,请填写完整");
      } else if (this.form.pass != this.form.passAgain) {
        this.$Message.warning("两次密码不一样,请确认");
      }else if( /^1[3|4|5|8][0-9]\d{8}$/.test(this.form.mobileNum) 
          && this.form.pass.length >= 6){
        this.$Message.warning("请输入正确的手机号和密码");
      }
       else {
        this.$http.post("http://localhost:3000/forget", {mobile: this.form.mobileNum,password: this.form.pass})
        .then(res => {
